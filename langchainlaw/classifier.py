...
class Classifier:
    ...
    def batch_check(
        ...
        """Check the status of a batch-submitted case, update the batch records spreadsheet, return the status and output_id for that case as a dict.
        Where a case and a prompt have produced multiple submitted batches, the status and output_id of the most recent submission results will be produced.
        """
        self.test = test
        case_id = casefile.stem
        self.load_judgment(casefile)
        ...
        if not self.test:
            ...
            #case_index_list = batch_records.index[batch_records['case_id']==case_id].tolist()
    ...
    def batch_retrieve_online(
        ...
        try:
            ...
            if status_output_id['status'] == 'completed':
                ...
                #Check if a JSON array is sought and returned but in the form of {{prompt.name}:[the JSON array]}
    ...
    def custom_id(
        ...
        """Generate a custom id for a case and a prompt.
