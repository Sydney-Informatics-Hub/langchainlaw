...
def process_prompts(input_file):
    ...
    prompt_responses = defaultdict(lambda: {'fields': defaultdict(dict), 'return_type': '', 'repeats': '', 'prompt_question': '', 'return_instruction': '', 'additional_instruction': ''})
    ...
    for i, row in prompts.iterrows():
        ...
        if row['prompt_question']:
            prompt_responses[row['Prompt_name']]['prompt_question'] = row['prompt_question']
        if row['return_instruction']:
            prompt_responses[row['Prompt_name']]['return_instruction'] = row['return_instruction']
        if row['additional_instruction']:
            prompt_responses[row['Prompt_name']]['additional_instruction'] = row['additional_instruction']
        
        prompt_responses[row['Prompt_name']]['fields'][row['fields']]['question_description'] = row['question_description']
        prompt_responses[row['Prompt_name']]['fields'][row['fields']]['example'] = row['example']
        prompt_responses[row['Prompt_name']]['fields'][row['fields']]['question_number'] = len(prompt_responses[row['Prompt_name']]['fields'])

    return prompt_responses


def generate_random_page_reference():
    return [f"(p{random.randint(1, 100)})", f"(pp{random.randint(1, 5)}-{random.randint(5, 10)})"][random.randint(0, 1)]
...
def assemble_prompts_to_yaml(prompts: dict):
    ...
    for prompt in prompts.keys():
        ...
        for field in prompts[prompt]['fields'].keys():
            result += f"      Q{prompts[prompt]['fields'][field]['question_number']}: {prompts[prompt]['fields'][field]['question_description']}\n"
        
        result += f"\n      {prompts[prompt]['return_instruction']}\n\n"
        
        # reconstruct a dictionary from the fields, keeping only the key and "example" value

        fields = {field: prompts[prompt]['fields'][field]['example'] + f" {generate_random_page_reference()}" for field in prompts[prompt]['fields'].keys()}
